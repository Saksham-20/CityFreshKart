...
/** Above MobileBottomNav (~4.5rem) + margin; cart has no nav but a tall sticky checkout bar (~7–8rem). */
const BOTTOM_OFFSET_DEFAULT = '5.5rem';
const BOTTOM_OFFSET_CART = '9.75rem';

const WhatsAppFloatingButton = () => {
  const { pathname } = useLocation();
  const isCart = pathname === '/cart';
  const bottomOffset = isCart ? BOTTOM_OFFSET_CART : BOTTOM_OFFSET_DEFAULT;
  ...
  return (
    <a
      ...
      className="fixed right-4 z-[60] w-14 h-14 rounded-full bg-[#25D366] text-white shadow-lg hover:scale-105 active:scale-95 transition-transform flex items-center justify-center"
